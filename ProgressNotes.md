...
To complement the automatic evaluation metrics, a logistic regression classifier was leveraged to assess whether the generated translations exhibit South Korean (SK) characteristics.

The classifier was trained using sentences from the test set, where North Korean (NK) sentences were labeled as 0 and South Korean reference sentences were labeled as 1. A character n-gram (1–3) representation was used to capture orthographic differences between the two variants. The classifier achieved an accuracy of approximately 91.7%, indicating that it was able to distinguish between NK and SK sentences.

The classifier was then applied to the model outputs to measure how well the generated translations were written in SK.

| Model | % classified as SK | Avg P(SK)|
|------|----------|---------------------|
| Baseline | 84.56 | 0.8289 |
| Aug_1.0x   | 85.71  | 0.8356 |
| Aug_2.0x  | 85.19  | 0.8351 | 
| Aug_3.0x | 85.36 | 0.8342 |
| Aug_4.0x | 84.67 | 0.8267 |

<br>
The logistic classifier results show that all models consistently generate outputs that are classified as South Korean at a high rate, indicating that the baseline model is already strong. However, increasing the amount of synthetic data did not lead to meaningful improvement.

<br>
The synthetic data, generated by the reverse SK->NK model, might not have introduced sufficiently diverse dialectal features beyond what is already captured in the original parallel corpus.

<br>
Moreover, because the classifier is based on character n-gram features that primarily captures surface-level orthographic patterns, it could've failed to detect improvements in higher-level syntactic, semantic, or stylistic aspects of the translations.

Together with the chrF and BERTScore results, the logistic classifier findings suggest that while semantic equivalence between the sentence pairs is well preserved, there is limited evidence that back-translation improves dialect-specific translation.

`./scripts/logistic_classifer.ipynb`
...
A qualitative analysis was conducted on the translation outputs from each model to examine which lexical and morphosyntactic differences between North and South Korean were successfully captured and which were not.

Since dealing with all sentence pairs was infeasible, a subset of 25 sentence pairs (that clearly exhibited linguistic differences between the two varieties) was manually selected from the test set.

These selected sentence pairs were translated using the baseline and augmented models (1×–4×), and the resulting outputs were saved for comparison.

`./src/translate_test.py` <br><br>
`./scripts/translate_selected_test_baseline.sh`
`./scripts/translate_selected_test_aug.sh`

input data : <br>
`./data/qualitative_analysis/selected_test.tsv`

output data : <br>
`./data/qualitative_analysis/aug_1.0x_nk_to_sk_selected_test.tsv`
`./data/qualitative_analysis/aug_2.0x_nk_to_sk_selected_test.tsv`
`./data/qualitative_analysis/aug_3.0x_nk_to_sk_selected_test.tsv`
